...
param_to_be_optimised_1 = 'voltage'

with open(file_name, 'a') as f:
		f.write('#'+ param_to_be_optimised_1 + ' ' + 'N_great' +  ' ' + 'N_less' + '\r\n')
f.close()
...
def func_g4(bounds):
	...
	path = fr'/home/mh1163604/low_energy/making_stopping_curves/Sandwich_in_SRIM/trim/1/chris_cooling/moving_decelerating/tuning_time_1/10_ns_reacc/using_python/dc_cell/over_500mm/5_mbar/volt_{str(bounds[0])}'
	path_main = '/home/mh1163604/low_energy/making_stopping_curves/Sandwich_in_SRIM/trim/1/chris_cooling/moving_decelerating/tuning_time_1/10_ns_reacc/using_python/dc_cell/over_500mm/5_mbar/'
	...
	g4_calc = subprocess.Popen(fr'apptainer run --app g4blmpi ~/g4blmpi_muoncooling_20250821.sif 16 input.g4bl E_field_magnitude={bounds[0]} > g4_out', shell=True)
	...
	N_less = (E < E_limit).sum()

	with open(file_name, 'a') as f:
		f.write(str(bounds[0]) + ' ' +  ' ' + str(N) + ' ' +  str(N_less) + '\r\n')
	f.close()
